...
(defn register [db work-factor {:keys [email username password confirm pgp-key]}]
  (if-let [errors (apply validate {:email email
    ...
    (do (add-user db work-factor email username password pgp-key)
        (workflow/make-auth {:identity username :username username}))))

(defn workflow [db work-factor]
  (fn [{:keys [uri request-method params]}]
    (when (and (= uri "/register")
               (= request-method :post))
      (register db work-factor params))))
